...
//		If the start and end vertex are equal, return a path containing one vertex and a cost of 0.
		...
		if (a.toString().equals(b.toString())) {
			...
			// start the dijkstra algorithm fresh without the modified values by initializing a new graph

			MyGraph graph = new MyGraph(this.collectionV, this.collectionE);
			shortest = dijkstra(graph,a,b);
		}
	...
	private Path dijkstra (MyGraph G, Vertex a, Vertex b) {
...
//		int n = 0;
		...
		//IF NOT COVERED PERFORM ALGORITHM
			...
			Collection<Vertex> Gvertices = G.collectionV;
...
//			System.out.println("\nPerforming Algorithm on Graph G");
			for (int i=0; i<n; i++) {
				...
				Iterator verticesInG = G.collectionV.iterator();
				Vertex U = null;
	...
	//				System.out.println("\nNow relaxing edges...");
					// relax all edges incident on Vertex U 
					Iterator edgesInG = G.collectionE.iterator();
	...
	//							currE.getSource().setDistance(updatedDistance);
								...
								System.out.println(currE.getSource().getLabel()+".descendant="+U.getLabel());
//								System.out.println("Since " + currE.getSource().getLabel() + " is a source than we don't update it's distance.");
							...
							if ((U.getDistance() + currE.getWeight()) < currE.getDestination().getDistance()) {
								...
								System.out.print(currE.getDestination().getLabel() + ".distance=" + updatedDistance +", "+currE.getDestination().getLabel()+".predecessor=("+U.toString()+"), ");
								System.out.println(U.getLabel() + ".descendant="+currE.getDestination().getLabel());
							}
			...
			while (traverseAdj.hasNext()) {
				...
				while (traverseDescendants.hasNext()) {
					...
					if (equalsB || alsoLeadsToB) {
						int descendantDistance = edgeCostNoOutput(desc.getPredecessor(),desc);
...
//						if (Ecost == -1) { totalDistance += descendantDistance; } // if there is no edge (starting node)
						totalDistance += descendantDistance ; 
					}
				}
				...
				if (u.toString().equals(B.toString()) || pathToBExist) {
					...
					// Check to ensure we do not return to a node already inside of Vertex List
					boolean exist = false;
					Iterator check = verticesAtoB.iterator();
					while (check.hasNext() ){
						Vertex currentVertex = (Vertex)check.next();
						if (currentVertex.toString().equals(goNext.toString())) {
							exist = true;
							break;
						}
					}
					
					if (totalDistance < minPathDistance && !exist) {
