...
public class FindPaths {
	...
	public static MyGraph readGraph(String f1, String f2) {
		...
		while(s.hasNext()) {
			try {
				// instead of creating a new vertex, just store the one that already exist
				Iterator vertices = v.iterator();
				
				Vertex aTemp = new Vertex(s.next());
				Vertex bTemp = new Vertex(s.next());
				...
				while (vertices.hasNext()) {
					...
					if (search.getLabel().equals(aTemp.getLabel()) && !aFound) {
						a = search;
					}
					// if we found b then store search in B
					if (search.getLabel().equals(bTemp.getLabel()) && !bFound) {
						b = search;
					}
				}
				if (a == null || b == null) {
					System.out.println("Error: We've found vertices in an edge that are not in your collection of vertices.");
					break;
				}
				int w = s.nextInt();
